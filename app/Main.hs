...
args = Args
  ...
  <*> (option auto
      ...
      <> metavar "PROVER"
      <> value Coq
      <> help "Prover for which code is generated. Choose Coq for well-scoped de Bruijn, UCoq for unscoped de Bruin. Default is well-scoped Coq code." ))
  <*> switch
      ( short 'f'
...
generate args sig = do
  ...
  case (dotFile args) of
    ...
    Nothing -> return ()
  let prettyCodeE = either Left (Right . ($ "") . displayS . renderPretty 1.0 (lineWidth args)) $ runGen sig (generateCode (prover args))
