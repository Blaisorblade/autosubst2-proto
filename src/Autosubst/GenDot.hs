...
sysFw = M.fromList [("kind", [star,karrow]), ("ty", [arrow,forall]), ("tm", [app,lam,tapp,tlam])]
  where pKind  = Position [] (Atom "kind")
        ...
        pAll   = Position ["ty"] (Atom "ty")
        pTAbs  = Position ["ty"] (Atom "tm")
        pAbs   = Position ["tm"] (Atom "tm")
...
genPositions tp ps = "FIXME" --formatTp (map genPosition ps) tp
--  where genPosition (Position bs dst) = maybeParens (not $ null bs) $ formatTp bs dst
