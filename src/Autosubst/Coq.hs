...
coqPreamble :: Doc
coqPreamble = text "Require Export fintype."
...
instance CoqShow MatchItem where
  coqShow (MatchItem s mt) = coqShow s
...
instance CoqShow Pattern where
  coqShow (PatCtor s ids) = coqShow s <+> hsep (map coqShow ids)
  coqShow (PatQualId s)   = coqShow s
  coqShow s               = text "Implement patterns."
...
instance CoqShow Sentence where
  ...
  coqShow (SentenceInstance bs x s t) = text "Instance" <+> text x <+> hsep (map coqShow bs) <+> text ":" <+> coqShow s <+> text ":=" <+> coqShow t <+> text "."
...
instance CoqShow Tactic where
  ...
  coqShow _ = text "MissingTacticImplementation"
...
instance CoqShow SubstTy where
  coqShow xs = hsep (map (\x -> case x of
                                                          ...
                                                          _ -> parens $ coqShow x) (sty_terms xs))
...
instance CoqShow Term where
  ...
  coqShow (TermMatch m _ s) = text "match" <+> coqShow m <+> text "with" <$$> vcat (map coqShow s) <$$> text "end" -- TODO: Version with Return clause
