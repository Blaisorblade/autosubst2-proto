...
directContainment spec = (g,v2id,id2v)
  where getArgs xs   = nub $ concatMap (map argument . positions) xs
...
buildSignature :: ([TId],Spec) -> Either String Signature
buildSignature (canonicalOrder, spec) =
  let
    (g, keyMap, lookUp) = directContainment spec
