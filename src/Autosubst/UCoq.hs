...
module Autosubst.UCoq where

-- Pretty Printing of Syntax to UCoq
import           Autosubst.Syntax
import           Text.PrettyPrint.Leijen

-- TODO: More documentation what is what.

-- TODO: These primitves are completely ignored.
...
ucoqPreamble :: Doc
ucoqPreamble = text "Require Export unscoped."
...
instance UCoqShow MatchItem where
  ucoqShow (MatchItem s mt) = ucoqShow s
...
instance UCoqShow Pattern where
  ucoqShow (PatCtor s ids) = ucoqShow s <+> hsep (map ucoqShow ids)
  ucoqShow (PatQualId s)   = ucoqShow s
  ucoqShow s               = text "Implement patterns."
...
instance UCoqShow Sentence where
  ...
  ucoqShow (SentenceCommand _) = text ""
  ucoqShow (SentenceNotation x t y z) = text "Notation" <+> dquotes (text x) <+> text ":=" <+> parens (ucoqShow t) <+> parens (text y) <+> text ":" <+> text z <> text "."
  ucoqShow (SentenceInstance bs x s t) = text "Instance" <+> text x <+> hsep (map ucoqShow bs) <+> text ":" <+> ucoqShow s <+> text ":=" <+> ucoqShow t <+> text "."
  ucoqShow (SentenceId s) = text s
  ucoqShow _                      = text "TODO: Other sentence forms."
...
instance UCoqShow Tactic where
  ...
  ucoqShow _ = text "Missing-tactic-implementation"
...
instance UCoqShow SubstTy where
  ucoqShow xs = hsep (map (\x -> case x of
                                                          ...
                                                          _ -> parens $ ucoqShow x) (sty_terms xs))
...
instance UCoqShow Term where
  ...
  ucoqShow (TermFunction s t) = ucoqShow s <+> text "->" <+> ucoqShow t
  ...
  ucoqShow (TermMatch m _ s) = text "match" <+> ucoqShow m <+> text "with" <$$> vcat (map ucoqShow s) <$$> text "end" -- TODO: Version with Return clause
