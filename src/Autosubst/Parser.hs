...
data Token = Ident String | Quotes | Comment String | TypeVariable String | LParen | RParen | KColon | KType | FType | KArrow deriving (Show, Eq)
...
identifier, typevariable, comment, lparen, rparen, ktype, ftype, karrow, kcolon :: Parser TokenPos
...
quotes = parsePos $ string "\"" >> return Quotes

token :: Parser TokenPos
token = choice [lparen, rparen, kcolon, try quotes, try ktype, try ftype, typevariable, identifier, try comment, karrow]
...
type SpecAST = ([TId], [FId], [ConstructorAST])
type ConstructorAST = (CId, TypeAST)
data FunctorAST = FunctorAST FId [FunctorAST] | AtomAST TId deriving Show
data TypeAST = FunAST FunctorAST | ArrowAST TypeAST TypeAST deriving Show
...
parseAtomicFType = (tok LParen *> parseFType <* tok RParen) <|> (AtomAST <$> parseId)
...
parseFType = (do
    ...
    x <- parseId
--    t <- parseAtomicFType
    ts <- many parseAtomicFType
    tok Quotes
    return $ FunctorAST x (ts)) <|> parseAtomicFType

-- constructor types
parseAtomicCType :: TokParser TypeAST
parseAtomicCType = (tok LParen *> parseCType <* tok RParen) <|> (FunAST <$> parseFType)
...
-- definitions
parseDef :: TokParser (Either (Either TId FId) (CId, TypeAST))
parseDef = (Left <$> ((Left <$> (try parseTypeDef)) <|> (Right <$> (try parseFunvarDef))) ) <|> (Right <$> parseConstructorDef)
...
parseSpec = (\(x,y) -> (fst (partitionEithers x), snd (partitionEithers x),y)) <$> partitionEithers <$> many parseDef
...
-- #TODO: Improve upon the AST. Not right as now.

checkArgument :: FunctorAST -> SA Argument
checkArgument (AtomAST x) = do
    checkTId x
    return $ Atom x
checkArgument (FunctorAST f xs) = do
...
checkPositions (FunAST (AtomAST x)) = return ([], x)
...
checkSpec :: SpecAST -> Either String ([TId],[FId],Spec)
checkSpec (tps, fs, cs) = do
...
parseFrom :: String -> String -> Either String ([TId],[FId],Spec)
parseFrom src input =
  case tokenize src input of
    Left err -> Left $ show err
    Right ts ->  either (Left . show) checkSpec $ runParser parseSpec () src . filter (not . isComment) $ ts
--    Right ts -> either (Left . show) (Left . show) $ runParser parseSpec () src . filter (not . isComment) $ ts -- : TEST to print the result.

parseFile :: FilePath -> IO (Either String ([TId],[FId],Spec))
parseFile f = do
  input <- readFile f
  return $ parseFrom f input

parseStdIn :: IO (Either String ([TId],[FId],Spec))
parseStdIn = do
  s <- getContents
